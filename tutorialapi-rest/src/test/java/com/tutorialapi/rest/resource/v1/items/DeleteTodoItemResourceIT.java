...
class DeleteTodoItemResourceIT extends BaseResourceIT {
...
//        return new ApiApplication(() -> new Environment(config, serviceFactory));
    ...
    @Test
    void testProxySecretAndUserHeader() {
        Response response = target("/v1/lists/list-id/items/item-id").request()
                .header(SecurityHeader.RAPID_API_PROXY_SECRET.getHeader(), "proxy-secret")
    ...
    void testMissingTodoItem() {
        ...
        Mockito.verify(
                todoItemService,
                Mockito.times(1)).delete(eq(principal),
                eq("list-id"),
                eq("item-id")
        );
    }
